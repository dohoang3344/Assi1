...
public class Registration extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (cusDAO.searchCustomerByUserName(username) == true) {
        ...
        try {
            if (username_err.length() == 0 && password_err.length() == 0 && name_err.length() == 0 && email_err.length() == 0 && address_err.length() == 0 && phone_err.length() == 0) {
                Customers cus = new Customers(username, password, name, phone, email, address,controller_id);
                cusDAO.addAccount(cus);
                url = "/Dangnhap.jsp";
            } else {
                url = "/Dangky.jsp";
            }
